# Issue #22: リーグ管理API実装

## ステータス
🔴 **Open**

## 優先度
🔥 **High**

## 概要

リーグ管理・プレイヤー管理APIを実装する。Hono RPC + 3層アーキテクチャで型安全なAPIを構築。

---

## 実装するAPI

### リーグ管理（6エンドポイント）
- `POST /api/leagues` - リーグ作成
- `GET /api/leagues` - リーグ一覧
- `GET /api/leagues/:id` - リーグ詳細
- `PATCH /api/leagues/:id` - リーグ更新
- `DELETE /api/leagues/:id` - リーグ削除（論理削除）
- `PATCH /api/leagues/:id/status` - ステータス変更

### プレイヤー管理（2エンドポイント）
- `PATCH /api/leagues/:id/players/:playerId` - プレイヤー名更新
- `PATCH /api/leagues/:id/players/:playerId/role` - 権限変更

---

## 実装の流れ

### ステップ1: 基盤準備
1. ディレクトリ構成作成（`src/server/`, `src/client/`）
2. データベース接続設定（`db/index.ts`）
3. 認証ミドルウェア作成（`src/server/middleware/auth.ts`）
4. エラーハンドラー作成（`src/server/middleware/error-handler.ts`）

### ステップ2: リーグ作成API（最重要）
1. バリデータ作成（`src/server/validators/leagues.validator.ts`）
2. リポジトリ作成（`src/server/repositories/leagues.repository.ts`）
3. サービス作成（`src/server/services/leagues.service.ts`）
4. ルート作成（`src/server/routes/leagues.ts`）
5. AppTypeエクスポート（`src/server/routes/index.ts`）

### ステップ3: 残りのエンドポイント
6. リーグ一覧、詳細、更新、削除、ステータス変更
7. プレイヤー名更新、権限変更

### ステップ4: フロントエンド統合
8. RPCクライアント初期化（`src/client/api.ts`）
9. React Query Hooks作成（`src/client/hooks/useLeagues.ts`）

---

## 重要な実装ポイント

### リーグ作成時の処理
1. **トランザクション処理**
   - リーグ作成 + プレイヤー作成を1トランザクションで実行

2. **作成者の権限付与**
   - 最初のプレイヤー（index: 0）を作成者と紐づけ
   - 自動的に `role: admin` を設定

3. **プレイヤー数検証**
   - 8人または16人のみ許可

### 権限チェック
- admin権限が必要な操作：リーグ更新、削除、ステータス変更、プレイヤー管理
- Service層で `hasAdminRole()` を実装

---

## 技術スタック

- Hono v4.10.4
- Zod + @hono/zod-validator
- Drizzle ORM v0.44.7
- Supabase Auth
- React Query

---

## 参考資料

- [api-design.md](../api-design.md)
- [directory-structure.md](../directory-structure.md)
- [Hono RPC](https://hono.dev/guides/rpc)

---

**作成日:** 2025-11-09
